...
public:
		...
		:Renderable(nullptr), nodeDepth_(maxDepth)
	{
		...
		std::vector<std::shared_ptr<Renderable>> renderables0, renderables1;

		for (const auto& r : renderables) 
		{
			AABB rend = r->getAABB();

			if (rend.centre()[splittingAxis] < splittingLoc) 
			{
				renderables0.push_back(r);
			}
			else 
			{
				renderables1.push_back(r);
			}
		}

		if (renderables0.empty() || renderables1.empty())
		{
			renderables0.clear();
			renderables1.clear();
			...
			{
				if (i % 2 == 0)
					renderables0.push_back(renderables[i]);
				else
					renderables1.push_back(renderables[i]);
			}
		}
		...
		if (maxDepth <= 0 || renderables.size() <= 2)
		{
			child0_ = std::make_shared<BVHLeafNode>(renderables);
		}
		else {
			child0_ = std::make_shared<BVHNode>(renderables0, maxDepth - 1);
			child1_ = std::make_shared<BVHNode>(renderables1, maxDepth - 1);
		}
	}
