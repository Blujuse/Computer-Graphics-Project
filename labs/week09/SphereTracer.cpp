...
bool raySphereIntersection(const Ray& ray, const Sphere& sphere, Vector3f& intersection, float& t, float minT=0.001f)
{
	...
	// WORKING OUT PROPERTIES OF QUADRATIC FUNCTION
	// A = 1; but we dont care RAAAAHHHHHH
	float B = 2 * (ray.origin - sphere.centre).dot(ray.direction); 
	// (sqrt(x^2 + y^2 + z^2))^2 - r^2
	float C = pow((ray.origin - sphere.centre).x(), 2) + pow((ray.origin - sphere.centre).y(), 2) + pow((ray.origin - sphere.centre).z(), 2) - sphere.radius;

	// WORKING OUT DISCRIMINANT, describes how many solutions of t there are
	float discriminant = pow(B, 2) - 4 * C;

	float solutionOne = (-B + sqrt(pow(B, 2) - 4 * C)) / 2;
	float solutionTwo = (-B - sqrt(pow(B, 2) - 4 * C)) / 2;

	// If t is greater than zero
	if (std::min(solutionOne, solutionTwo) > minT)
	{
		if (discriminant > 0)
		{
			// Two Solutions
			t = std::min(solutionOne, solutionTwo);
...
bool refract(const Vector3f& incident, const Vector3f& norm, float eta, Vector3f& refracted)
{
	...
	float k = 1 - pow(eta, 2) * (1 - pow(incident.dot(norm), 2));

	if (k < 0) // less than zero, total internal reflection
	{
		return false;
	}

	// Work out refraction
	refracted = (eta * incident) - norm * ((incident.dot(norm) + sqrt(k)));
	...
	// and a coefft-wise product with the albedo.
		...
		for (const auto& light : lights) {
			if (light->getType() == Light::AMBIENT) {
				...
				float dotProd = -lightDir.dot((*hitSphere, hitIntersection));
