...
Eigen::Matrix4f projectionMatrix(int height, int width, float horzFov = 70.f*M_PI/180.f, float zFar = 10.f, float zNear = 0.1f)
{
	...
	double vertFov = 2 * atan(tan(horzFov / 2));
	// Now construct the matrix.
	Eigen::Matrix4f projection;
	double tanH = 1 / tan(horzFov / 2);
	double tanV = 1 / tan(vertFov / 2);
	...
	int width, int height)
{
	for (int i = 0; i < mesh.vFaces.size(); ++i) {
		...
		t.screen[0].x() = ((vClip0.x() + 1) / 2) * width;
		t.screen[0].y() = ((-vClip0.y() + 1) / 2) * height;
		t.screen[0].z() = vClip0.z();

		t.screen[1].x() = ((vClip1.x() + 1) / 2) * width;
		t.screen[1].y() = ((-vClip1.y() + 1) / 2) * height;
		t.screen[1].z() = vClip1.z();

		t.screen[2].x() = ((vClip2.x() + 1) / 2) * width;
		t.screen[2].y() = ((-vClip2.y() + 1) / 2) * height;
