	...
	for(int y = 0; y < height; ++y) 
		for (int x = 0; x < width; ++x) 
			for (int c = 0; c < 3; ++c) {
				...
				// Literally just reverse max value thingy can be
				// normalize with 255, then once thats done you can multiply by any number less than or equal to one to then make it go brighter proportionally
				//imageBuffer[pixelIdx * 4 + c] = 0 - imageBuffer[pixelIdx * 4 + c] * -1;
				imageBuffer[pixelIdx * 4 + c] = 255 - imageBuffer[pixelIdx * 4 + c] * 1;
			}
