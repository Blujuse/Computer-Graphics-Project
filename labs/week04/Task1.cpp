	...
	int width, int height)
{
	for (const auto& face : mesh.faces) {
		Eigen::Vector4f
			v0 = vec3ToVec4(mesh.verts[face[0]]),
			v1 = vec3ToVec4(mesh.verts[face[1]]),
			v2 = vec3ToVec4(mesh.verts[face[2]]);
...
#pragma region LearnEigen
	...
	std::cout << "Subtask 3:" << std::endl;
	std::cout << myThreeByThreeMatrix * myInverse << std::endl;
...
#pragma endregion
	...
	Eigen::Matrix4f bunnyTransform = translationMatrix(Eigen::Vector3f(-0.5, -0.5,0));
	Eigen::Matrix4f dragonTransform = translationMatrix(Eigen::Vector3f(0.5, 0.1, 0));
	dragonTransform *= rotateYMatrix(180.0f);

	Eigen::Matrix4f armadilloTransform = translationMatrix(Eigen::Vector3f(0.5, -0.3, 0));
	...
	drawMesh(imageBuffer, armadilloMesh, Eigen::Vector3f(0, 1, 1), armadilloTransform, width, height);
