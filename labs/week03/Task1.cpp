	...
	uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255)
{
	...
	// YOUR CODE HERE
	if (0 <= minX > width || 0 <= minY > height || 0 <= maxX > width || 0 <= maxY > height)
	{
	...
	triangleArea = abs(edge1.cross(edge2) / 2);
	...
	for(float x = minX; x <= maxX; ++x) 
		for (float y = minY; y <= maxY; ++y) {
			...
			a0 = abs(edge1.cross(edge2) / 2);
			a1 = abs(edge2.cross(edge3) / 2);
			a2 = abs(edge3.cross(edge1) / 2);
...
int main()
{
	...
	for (const auto& face : faces) {
		...
		drawTriangle(imageBuffer, width, height, p0, p1, p2, rand() % 105 + 150, rand() % 255, rand() % 255, 255);
